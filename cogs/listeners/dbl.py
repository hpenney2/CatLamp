    ...
    async def webserver(self):
        async def handleDBLVote(request):
            ...
            if auth != "Imagin3ActuallyHavingGo0dSEcuR1tyF0rCATLAMP":
                ...
                return web.json_response({"error": "invalid_auth_header"}).set_status(400)
            else:
                ctype = request.headers.get("Content-Type")
                if ctype != "application/json":
                    print(f"DBL POST request failed due to not having a JSON content type!\nContent-Type header: {ctype}")
                    return web.json_response({"error": "bad_content_type"}).set_status(400)
                ...
                try:
                    ...
                    return web.json_response({"error":"missing_keys"}).set_status(400)
                if postType == "test":
                    ...
                    return web.json_response({"error":"bad_vote_type"}).set_status(400)
        app = web.Application()
